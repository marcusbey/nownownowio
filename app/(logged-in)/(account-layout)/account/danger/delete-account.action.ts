...
import AccountAskDeletionEmail from "@/emails/account-ask-deletion.email";
import AccountConfirmDeletionEmail from "@/emails/account-confirm-deletion.email";
...
export const accountAskDeletionAction = authAction.action(async ({ ctx }) => {
  ...
  await sendEmail({
    ...
    react: AccountAskDeletionEmail({
      organizationsToDelete: user.memberships.map(
        (o: { organization: { name: string } }) => o.organization.name,
      ),
