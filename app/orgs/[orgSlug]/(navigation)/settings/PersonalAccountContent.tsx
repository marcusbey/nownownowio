...
export function PersonalAccountContent({
  user,
  isEmailVerified,
}: PersonalAccountContentProps) {
  return (
    <div className="mx-auto max-w-4xl py-6">
      ...
      {!isEmailVerified && user.email && (
        <Alert className="mb-6">
          <AlertDescription className="flex items-center justify-between">
            <span>
              Your email address ({user.email}) is not verified. Please verify
              your email to ensure account security.
            </span>
            <Button variant="outline" size="sm" asChild>
              <Link
                href={`/api/auth/verify-email?email=${encodeURIComponent(user.email)}`}
              >
                Verify Email
              </Link>
            </Button>
      ...
      <Card className="border shadow-sm">
        <CardHeader>
          ...
          <CardDescription>Customize your profile banner image</CardDescription>
        </CardHeader>
        <CardContent>
