...
export function getOrganizationNavigation(
    ...
    return ORGANIZATION_LINKS.map((group: NavigationGroup) => ({
        ...
        defaultOpenStartPath: group.defaultOpenStartPath
            ...
            : undefined,
        links: group.links
            .filter((link) => (link.roles ? isInRoles(userRoles, link.roles) : true))
            .map((link) => ({ ...link, href: replaceSlug(link.href, slug) }))
    }));
}
...
export const ORGANIZATION_LINKS: NavigationGroup[] = [
    {
        title: "ACTIVITIES",
        links: [
            { href: ORGANIZATION_PATH, icon: "home", label: "Home" },
            { href: prependOrgPath("/explore"), icon: "compass", label: "Explore" },
            { href: prependOrgPath("/profile"), icon: "user", label: "Profile" },
            { href: prependOrgPath("/notifications"), icon: "bell", label: "Notifications" },
            { href: prependOrgPath("/bookmarks"), icon: "bookmark", label: "Bookmarks" }
        ]
    },
    {
        title: "SETTINGS",
        links: [
            { href: prependOrgPath("/panel"), icon: "panel-left", label: "Panel", roles: ["OWNER", "ADMIN"] },
            { href: prependOrgPath("/dashboard"), icon: "layout-dashboard", label: "Dashboard", roles: ["OWNER", "ADMIN"] },
            { href: prependOrgPath("/settings"), icon: "settings", label: "Settings", roles: ["OWNER", "ADMIN"] }
        ]
    }
];
